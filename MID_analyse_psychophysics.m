...
for subject = 1:4
    for signal = 1:2
        ...
        % We need to replace the NaNs in the resampled CD data with zeros
...
for signal = 1:2 % loop across coherence and contrast
    ...
    % Find the coefficients (the CD & IOVD weights). For now just do the straight fit to get overall weights on the CD and IOVD
    % Using a linear backslash fit
    % This is model 1, without the betas.
    w_m1_av_data(:, signal) = [CD_IOVD_av.Sens{signal}(:), CD_IOVD_av.Sens{signal+2}(:)] \ FULLcue_av.MeanSensData{signal}(:); % [CD,IOVD] \ FULL
    
    % Now solve model 2, which includes the beta values.
    w_m2_av_data(:, signal) = [CD_IOVD_av.Sens{signal}(:).*CD_IOVD_av.Betas{signal}(:), ... % CD
        CD_IOVD_av.Sens{signal+2}(:).*CD_IOVD_av.Betas{signal+2}(:)] ... % IOVD
        \ FULLcue_av.MeanSensData{signal}(:);
    ...
    % Compute MSE for model 1, the simpler model without the betas:
    % Remember to divide by N to give the MEAN squared error.
    MSE_1 = sum((FULLcue_av.MeanSensData{signal}(:) - predictions(:,1)).^2) / length(FULLcue_av.MeanSensData{signal}(:));
    
    % Now do the same for model 2. This includes the alphas weighted by the betas, and the fitted weights.
    MSE_2 = sum((FULLcue_av.MeanSensData{signal}(:) - predictions(:,2)) .^2) / length(FULLcue_av.MeanSensData{signal}(:));
    ...
    GofFit_av_data(signal,1) = goodnessOfFit(predictions(:,1), FULLcue_av.MeanSensData{signal}(:), 'NMSE');
    GofFit_av_data(signal,2) = goodnessOfFit(predictions(:,2), FULLcue_av.MeanSensData{signal}(:), 'NMSE');
...
for subject = 1:4 % loop across subjects
    for signal = 1:2 % loop across coherence and contrast
        ...
        % Using a linear backslash fit
        % This is model 1, without the betas.
        w_m1(:, signal, subject) = [CD_IOVD{subject}.Sens{signal}(:), CD_IOVD{subject}.Sens{signal+2}(:)] ...
            \  FULLcue{subject}.MeanSensData{signal}(:); % [CD,IOVD] \ FULL
        
        % Now solve model 2, which includes the beta values.
        w_m2(:, signal, subject) = [CD_IOVD{subject}.Sens{signal}(:).*CD_IOVD{subject}.Betas{signal}(:), ... % CD
            CD_IOVD{subject}.Sens{signal+2}(:).*CD_IOVD{subject}.Betas{signal+2}(:)] ... % IOVD
            \ FULLcue{subject}.MeanSensData{signal}(:);
        ...
        % Compute MSE for model 1, the simpler model without the betas:
        % Remember to divide by N to give the MEAN squared error.
        MSE_1(subject) = sum((FULLcue{subject}.MeanSensData{signal}(:) - predictions(:,1)).^2) / length(FULLcue{subject}.MeanSensData{signal}(:));
        
        % Now do the same for model 2. This includes the alphas weighted by the betas, and the fitted weights.
        MSE_2(subject) = sum((FULLcue{subject}.MeanSensData{signal}(:) - predictions(:,2)) .^2) / length(FULLcue{subject}.MeanSensData{signal}(:));
        
        % Now we could do a chi^2 goodness of fit between the data and the predictions
        % NMSE costs vary between -Inf (bad fit) to 1 (perfect fit). If the cost function is equal to zero, then x is no better than a straight line at matching xref.
        GofFit(signal,1,subject) = goodnessOfFit(predictions(:,1), FULLcue{subject}.MeanSensData{signal}(:), 'NMSE');
        GofFit(signal,2,subject) = goodnessOfFit(predictions(:,2), FULLcue{subject}.MeanSensData{signal}(:), 'NMSE');
...
save('MID_psychophysics_modelling_results.mat', 'GofFit_av_data', 'w_m1_av_data', 'w_m2_av_data', ...
